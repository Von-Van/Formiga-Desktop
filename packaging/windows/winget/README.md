...
1. Make sure the release is actually published with its Windows assets, e.g.:
   ```sh
   gh release view v0.58.8 --json assets
   ```
2. Generate the manifest:
   ```sh
   scripts/winget-manifest.sh 0.58.8
   ```
...
5. Fork `microsoft/winget-pkgs`, copy the three generated files into
   `manifests/v/VonVan/Formiga/0.58.8/` in that fork (that path - lowercase first letter of the
